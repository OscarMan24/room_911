                ...
                <div class="card-body">
                    ...
                    <div class="row col-12 mb-4">
                        ...
                        <div class="col-lg-3 col-md-4 col-12 mb-2">
                            ...
                            <select class="form-control @error('search_employees') is-invalid @enderror"
                                wire:model="search_employees">
                                <option value="1">{{ __('Employees found') }}</option>
                                <option value="2">{{ __('Employees not found') }}</option>

                            </select>
                            @error('search_employees')
                                <div class="invalid-feedback ">{{ $message }} </div>
                            @enderror
                        </div>
                        <div class="col-lg-3 col-md-4 col-12 mb-2">
            ...
            <div class="card">
                <div class="card-body">
                                ...
                                <tbody>
                                    @forelse ($this->Records as $record)
                                        <tr>
                                            ...
                                            <td>{{ Carbon\Carbon::create($record->created_at)->isoFormat('LLLL') }}
                                            </td>
                    ...
                    @else
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        <th scope="col">#</th>
                                        <th scope="col">{{ __('Ident') }}</th>
                                        <th scope="col">{{ __('Date') }}</th>
                                        <th scope="col">{{ __('Access') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse ($this->RecordsNotFound as $record)
                                        <tr>
                                            <th scope="row">#{{ $record->id }}</th>
                                            <td>
                                                {{ $record->read_code }}
                        ...
                        @if (count($this->RecordsNotFound) > 0)
                            <div class="row text-center justify-content-center mt-2" style="max-width: 99%">
                                {{ $this->RecordsNotFound->onEachSide(1)->links() }}
                            </div>
                        @endif
                    @endif
